...
def detect(
    ...
    print(f"Output type: {type(out)}")  # 打印 out 的类型
    if isinstance(out, (tuple, list)):  # 如果 out 是元组或列表
        print(f"Output is a tuple/list with {len(out)} elements:")
        for i, o in enumerate(out):
            print(f"  Output {i} shape: {o.shape}")  # 打印每个元素的形状
            print(f"  Output {i} content (first 5 values): {o.asnumpy()[:5]}")  # 打印前 5 个值
    else:  # 如果 out 是单个张量
        print(f"Output shape: {out.shape}")  # 打印 out 的形状
        out_np= out.asnumpy()
        non_nan_values = out_np[~np.isnan(out_np)]
        print(f"Output content (first 5 values): {non_nan_values[:10]}")  # 打印前 5 个值
    # Run NMS
    t = time.time()
    out = out.asnumpy()
